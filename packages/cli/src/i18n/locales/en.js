...
export default {
  ...
  WITTY_LOADING_PHRASES: [
    ...
    // DevOps / SRE
    'The pager duty alert paged duty',
    'kubectl describe the describe',
    'The CI pipeline is green, but at what cost',
    'Adding a sleep 30 to the deployment script',
    'The cron job cron-jobbed itself',
    'Restarting the server since 2017',
    'The load balancer is load-balancing the blame',
    'docker-compose up -d --force-recreate --prayer',
    'The Terraform plan plans to plan again',
    'Has it tried turning off and on again',
    'The canary deployment canaried',
    'Rolling back the rollback that rolled back',
    'The Kubernetes pod is in CrashLoopBackOff of life',
    'Adding monitoring to the monitoring system',
    ...
    'The health check is checking the health of the check',
    'rm -rf node_modules and hoping for the best',
    'The database migration is migrating the database',
    'The feature flag flagged a feature',
    'Nmap found a port, were all panicked',

    // Debugging nightmares
    'It works on my machine, legally binding',
    'The bug is a feature that filed early',
    'Rubber ducking the rubber duck',
    'The stack trace traces a stack of lies',
    'Adding console.log because the debugger is lying',
    'The off-by-one error is off by two',
    'The race condition raced and won',
    'Memory leak leaking memory about the leak',

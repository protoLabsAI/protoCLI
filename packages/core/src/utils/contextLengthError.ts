...
/**
 * Provider-agnostic classification of "context length exceeded" errors.
 *
 * Different providers report an over-budget request in very different shapes
 * (OpenAI: "maximum context length is N tokens ... requested M"; Anthropic /
 * others: "prompt is too long", "too many tokens", "tokens > N"). This module
 * normalises an arbitrary error value into a single verdict the chat layer can
 * act on — used to trigger reactive compression-and-retry. Ported from
 * QwenLM/qwen-code#3879.
 */
...
/**
 * Inspect an arbitrary error value (including nested causes and embedded JSON
 * error bodies) and decide whether it represents a context-length-exceeded
 * rejection — distinct from a timeout, which can carry similar wording but
 * must NOT trigger compression. Returns parsed token counts when the provider

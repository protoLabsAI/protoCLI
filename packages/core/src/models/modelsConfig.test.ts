...
describe('ModelsConfig', () => {
  ...
  describe('getAllConfiguredModels', () => {
    ...
    it('should support filtering by authTypes', () => {
      ...
      const openaiAndAnthropic = modelsConfig.getAllConfiguredModels([
        AuthType.USE_OPENAI,
        AuthType.USE_ANTHROPIC,
      ]);
      expect(openaiAndAnthropic.length).toBe(2);
      expect(openaiAndAnthropic.map((m) => m.id)).toContain('openai-model-1');
      expect(openaiAndAnthropic.map((m) => m.id)).toContain(
        'anthropic-model-1',
      );
    });
  });

...
- **Mouse + selection escape hatch:** SGR `?1006` wheel parsing in `KeypressContext`; **OSC-52 yank** for copy; a one-key **drop-to-inline** toggle that emits `\x1b[?1049l`, prints the transcript slice to the normal buffer for native select/copy, and re-enters on keypress.
...
| Always-on mouse capture in inline mode                                                                             | Steals native text selection/copy (Codex #1247, Gemini backlash). Mouse lives only inside full-screen mode, paired with OSC-52 yank + a one-key drop-to-inline escape hatch.                                                                   |
